...
def learn(env,
    ...
    Q_targ = q_func(obs_tp1_float, num_actions, scope="target_q_func_vars", reuse=False);
    opt_a = tf.argmax(Q_targ,1);
    #mask = tf.one_hot(opt_a,num_actions);
    opt_a_val = tf.reduce_max(Q_targ,reduction_indices=1);
    y = rew_t_ph + gamma*tf.multiply((1.0-done_mask_ph),opt_a_val);
 
    maskk = tf.one_hot(act_t_ph,num_actions);
    Q_ = tf.reduce_sum(tf.multiply(Q,maskk),axis=1); 

    total_error = tf.reduce_mean(tf.square(y-Q_));
    ...
    ### NOISE PARAMS ###
    origin = [10,30]#[25,10]; #[42,10]
    h_and_w = [20,50]#[70,15]; #[42,20]
    ####################
    ...
    for t in itertools.count():
        #if(np.mod(t,100) == 0): print(str(t));
        ### 1. Check stopping criterion
        if stopping_criterion is not None and stopping_criterion(env, t):
            pickle.dump(session.run(q_func_vars),open("expert_np_"+str(origin)+"_"+str(h_and_w)+".pkl","wb"));
            break
        ...
        if(eps < exploration.value(t) or model_initialized == False):
            action = np.random.randint(num_actions);
        else:
            action = session.run(opt_a,{obs_tp1_ph:p_obs[None],})
        ...
        if (t > learning_starts and
            ...
            if(model_initialized == False): 
                ...
                session.run(train_fn,{learning_rate:optimizer_spec.lr_schedule.value(t),obs_t_ph:obs_t_batch,act_t_ph:act_batch,rew_t_ph:rew_batch,obs_tp1_ph:obs_tp1_batch,done_mask_ph:done_mask})
